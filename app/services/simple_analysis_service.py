...
def get_provider_and_url_by_model_sync(model_name: str) -> dict:
    ...
    try:
        ...
        try:
            ...
            if db is None:
                logger.warning("MongoDB同步连接不可用，使用默认模型映射")
            ...
            logger.warning(f"⚠️ [同步查询] 数据库查询失败: {e}")
...
def _get_default_provider_by_model(model_name: str) -> str:
    ...
    if model_name in model_provider_map:
        ...
        provider = 'ollama'
    elif any(p in model_name.lower() for p in ['instruct', 'it', 'chat']):
        if any(k in model_name.lower() for k in ['qwen', 'llama', 'gemma', 'mistral', 'deepseek']):
            provider = 'lmstudio'
        else:
            provider = 'qwen'
    else:
...
class SimpleAnalysisService:
    ...
    async def _update_progress_async(self, task_id: str, progress: int, message: str):
        ...
        try:
            ...
            if db is not None:
                ...
                logger.debug(f"✅ [异步更新] 已更新内存(MongoDB不可用): {progress}%")
        except Exception as e:
    ...
    async def create_analysis_task(
        ...
        try:
            ...
            try:
                db = get_mongo_db()
                if db is None:
                    logger.warning("MongoDB连接不可用，跳过任务记录保存")
                else:
    ...
    async def execute_analysis_background(
        ...
        try:
            ...
            current_provider = DEFAULT_CONFIG.get("llm_provider", "lmstudio")
            ...
            if hasattr(request, 'parameters') and request.parameters:
                if hasattr(request.parameters, 'quick_analysis_model'):
                    error_context['model'] = request.parameters.quick_analysis_model
                if hasattr(request.parameters, 'deep_analysis_model'):
                    error_context['model'] = request.parameters.deep_analysis_model
            try:
                from app.core.unified_config import unified_config
                _llm_configs = unified_config.get_llm_configs()
                _model_name = error_context.get('model') or (request.parameters.quick_analysis_model if request.parameters else None)
                if _model_name:
                    for _cfg in _llm_configs:
                        if _cfg.model_name == _model_name:
                            error_context['llm_provider'] = _cfg.provider
                            break
                if not error_context.get('llm_provider'):
                    _default_model = unified_config.get_default_model()
                    if _default_model:
                        for _cfg in _llm_configs:
                            if _cfg.model_name == _default_model:
                                error_context['llm_provider'] = _cfg.provider
                                break
            except Exception:
                pass
    ...
    def _run_analysis_sync(
        ...
        try:
            ...
            def simulate_progress():
                ...
                try:
                    ...
                    for i, analyst in enumerate(analysts):
                        ...
                        elif analyst == "warren_buffett":
                            progress_tracker.update_progress("🎩 巴菲特大师正在分析")
                        elif analyst == "peter_lynch":
                            progress_tracker.update_progress("🎩 彼得·林奇大师正在分析")
                        elif analyst == "ben_graham":
                            progress_tracker.update_progress("🎩 格雷厄姆大师正在分析")
                        elif analyst == "charlie_munger":
                            progress_tracker.update_progress("🎩 芒格大师正在分析")
                        elif analyst == "cathie_wood":
                            progress_tracker.update_progress("🎩 凯瑟琳·伍德大师正在分析")
                        elif analyst == "bill_ackman":
                            progress_tracker.update_progress("🎩 阿克曼大师正在分析")
                        elif analyst == "phil_fisher":
                            progress_tracker.update_progress("🎩 费舍尔大师正在分析")
                        elif analyst == "stanley_druckenmiller":
                            progress_tracker.update_progress("🎩 德鲁肯米勒大师正在分析")
            ...
            try:
                ...
                master_reports_dict = {}
                if isinstance(state, dict) and "master_reports" in state:
                    master_reports_dict = state["master_reports"]
                elif hasattr(state, "master_reports"):
                    master_reports_dict = getattr(state, "master_reports", {})

                # 诊断日志：记录 master_reports 的详细内容
                if isinstance(master_reports_dict, dict):
                    logger.info(f"📊 [REPORTS] master_reports字典包含 {len(master_reports_dict)} 个key: {list(master_reports_dict.keys())}")
                    for mid, mtext in master_reports_dict.items():
                        text_len = len(mtext.strip()) if isinstance(mtext, str) else 0
                        logger.info(f"📊 [REPORTS] master_reports[{mid}] 长度: {text_len}")
                else:
                    logger.warning(f"⚠️ [REPORTS] master_reports不是字典: {type(master_reports_dict)}")

                if isinstance(master_reports_dict, dict):
                    for master_id, report_text in master_reports_dict.items():
                        report_key = f"{master_id}_report"
                        if report_key not in reports and isinstance(report_text, str) and len(report_text.strip()) > 10:
                            reports[report_key] = report_text.strip()
                            logger.info(f"📊 [REPORTS] 从master_reports提取: {report_key} - 长度: {len(report_text.strip())}")
                        elif report_key in reports:
                            logger.debug(f"📊 [REPORTS] 跳过已存在的报告: {report_key}")
                        elif not isinstance(report_text, str):
                            logger.warning(f"⚠️ [REPORTS] 跳过非字符串报告: {report_key} - 类型: {type(report_text)}")
                        elif len(report_text.strip()) <= 10:
                            logger.warning(f"⚠️ [REPORTS] 跳过过短的报告: {report_key} - 长度: {len(report_text.strip())}")
            ...
            risk_score = formatted_decision.get("risk_score", 0.5) if isinstance(formatted_decision, dict) else 0.5
            if risk_score < 0.3:
                risk_level = "低"
            elif risk_score < 0.7:
                risk_level = "中等"
            ...
            try:
                from app.core.unified_config import unified_config
                _llm_configs = unified_config.get_llm_configs()
                _model_name = error_context.get('model') or (request.parameters.quick_analysis_model if request and hasattr(request, 'parameters') and request.parameters else None)
                if _model_name:
                    for _cfg in _llm_configs:
                        if _cfg.model_name == _model_name:
                            error_context['llm_provider'] = _cfg.provider
    ...
    async def cleanup_zombie_tasks(self, max_running_hours: int = 2) -> Dict[str, Any]:
        ...
        try:
            ...
            cutoff_time = datetime.now(timezone.utc) - timedelta(hours=max_running_hours)
    ...
    async def _save_analysis_result_web_style(self, task_id: str, result: Dict[str, Any]):
        ...
        try:
            ...
            # 生成分析ID（与web目录保持一致）
            from datetime import datetime
            timestamp = datetime.now(timezone.utc)  # 存储 UTC 时间（标准做法）
            ...
            if 'state' in result:
                try:
                    ...
                    report_fields = [
                        ...
                        'peter_lynch_report',
                        'ben_graham_report',
                        'charlie_munger_report',
                        'cathie_wood_report',
                        'bill_ackman_report',
                        'phil_fisher_report',
                        'stanley_druckenmiller_report',
                        'aswath_damodaran_report',
                        'michael_burry_report',
                        'mohnish_pabrai_report',
                        'nassim_taleb_report',
                        'rakesh_jhunjhunwala_report',
                        'master_consensus_report',
                        'china_market_report',
                    ]
        ...
        try:
            ...
            if isinstance(master_reports_dict, dict):
                master_display_names = {
                    "warren_buffett": "巴菲特", "peter_lynch": "彼得·林奇",
                    "ben_graham": "格雷厄姆", "charlie_munger": "芒格",
                    "cathie_wood": "凯瑟琳·伍德", "bill_ackman": "阿克曼",
                    "phil_fisher": "费舍尔", "stanley_druckenmiller": "德鲁肯米勒",
                    "aswath_damodaran": "达莫达兰", "michael_burry": "布瑞",
                    "mohnish_pabrai": "帕伯莱", "nassim_taleb": "塔勒布",
                    "rakesh_jhunjhunwala": "朱朱瓦拉",
                }
                for master_id, report_text in master_reports_dict.items():
                    if isinstance(report_text, str) and len(report_text.strip()) > 10:
                        try:
                            display_name = master_display_names.get(master_id, master_id.replace("_", " ").title())
                            filename = f"{master_id}_report.md"
                            file_path = reports_dir / filename
                            with open(file_path, 'w', encoding='utf-8') as f:
                                f.write(report_text)
                            saved_files[f"{master_id}_report"] = str(file_path)
                            logger.info(f"✅ 保存大师报告: {file_path} ({display_name})")
                        except Exception as e:
                            logger.warning(f"⚠️ 保存大师报告 {master_id} 失败: {e}")

            master_consensus = state.get("master_consensus_report", "") if isinstance(state, dict) else ""
            if master_consensus and len(master_consensus.strip()) > 10:
                try:
                    file_path = reports_dir / "master_consensus_report.md"
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write(master_consensus)
                    saved_files["master_consensus_report"] = str(file_path)
                    logger.info(f"✅ 保存大师共识报告: {file_path}")
                except Exception as e:
                    logger.warning(f"⚠️ 保存大师共识报告失败: {e}")
