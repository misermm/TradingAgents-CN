...
class DataSourceManager:
    ...
    def close(self):
        """Best-effort cleanup for adapters and shared sync database resources."""
        for adapter in self.adapters:
            provider = getattr(adapter, "_provider", None)
            close_method = getattr(provider, "close", None) if provider is not None else None
    ...
    def _load_priority_from_database(self):
        ...
        try:
            ...
            if db is None:
                logger.info("MongoDB unavailable; using default A-share source priorities")
                for adapter in self.adapters:
