...
class ConfigService:
    ...
    async def update_category_datasource_order(self, category_id: str, ordered_datasources: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            logger.exception(f"❌ 更新分类数据源排序失败: {e}")
            return False
    ...
    async def delete_llm_config(self, provider: str, model_name: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"❌ 删除LLM配置失败: {e}")
            return False
    ...
    async def test_llm_config(self, llm_config: LLMConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            is_local_provider = provider_str in ("ollama", "lmstudio") or (
                provider_data and provider_data.get("extra_config", {}).get("is_local_provider", False)
            )
            ...
            if not is_local_provider:
                ...
                logger.info(f"✅ 本地模型提供商 {provider_str}，无需 API Key")
            ...
            if ds_type == "tushare":
                ...
                if ds_config.endpoint:
                    try:
                        ...
                        response = await asyncio.to_thread(requests.get, ds_config.endpoint, params=params, headers=headers, timeout=10)
                        response_time = time.time() - start_time
    ...
    async def add_database_config(self, db_config: DatabaseConfig) -> bool:
        ...
        try:
            ...
            logger.exception(f"❌ 添加数据库配置失败: {e}")
            return False
    ...
    async def update_database_config(self, db_config: DatabaseConfig) -> bool:
        ...
        try:
            ...
            logger.exception(f"❌ 更新数据库配置失败: {e}")
            return False
    ...
    async def delete_database_config(self, db_name: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"❌ 删除数据库配置失败: {e}")
            return False
    ...
    async def delete_llm_provider(self, provider_id: str) -> bool:
        ...
        try:
            ...
            if not existing:
                logger.warning(f"删除厂家失败: 找不到厂家 {provider_id}")
                return False
            ...
            logger.info(f"删除厂家结果: {success}, provider_id: {provider_id}")
            return success
    ...
    async def init_aggregator_providers(self) -> Dict[str, Any]:
        ...
        Returns:
            初始化结果统计
        """
        from app.constants.model_capabilities import AGGREGATOR_PROVIDERS, LOCAL_MODEL_PROVIDERS
        ...
        try:
            ...
            for provider_name, config in LOCAL_MODEL_PROVIDERS.items():
                existing = await providers_collection.find_one({"name": provider_name})

                if existing:
                    skipped_count += 1
                    logger.info(f"⏭️ 本地模型厂家 {config['display_name']} 已存在，跳过")
                    continue
                ...
                provider_data = {
                    ...
                    "default_base_url": config["default_base_url"],
                    "is_active": True,
                    "supported_features": config.get("supported_features", ["chat", "completion"]),
                    "api_key": "",
                    "extra_config": {
                        "has_api_key": False,
                        "source": "local",
                ...
                await providers_collection.insert_one(insert_data)
                added_count += 1
                logger.info(f"✅ 添加本地模型厂家: {config['display_name']}")

            message_parts = []
            if added_count > 0:
                message_parts.append(f"成功添加 {added_count} 个厂家")
            if updated_count > 0:
            ...
            logger.exception(f"❌ 初始化聚合渠道失败: {e}")
            return {
                "success": False,
                "error": str(e),
                "message": "初始化聚合渠道失败"
    ...
    async def test_provider_api(self, provider_id: str) -> dict:
        ...
        try:
            ...
            if not self._is_valid_api_key(api_key):
                ...
                if env_api_key:
                    api_key = env_api_key
                    logger.info(f"✅ 数据库配置无效，从环境变量读取到 {display_name} 的 API Key")
                else:
                    return {
                        "success": False,
                ...
                logger.info(f"✅ 使用数据库配置的 {display_name} API密钥")
    ...
    def _test_google_api(self, api_key: str, display_name: str, base_url: str = None, model_name: str = None) -> dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result = response.json()
                logger.info(f"📥 [Google AI 测试] 解析后的 JSON 结构:")
    ...
    def _test_deepseek_api(self, api_key: str, display_name: str, model_name: str = None) -> dict:
        ...
        try:
            ...
            if api_key and api_key not in ("ollama", "lmstudio"):
                headers["Authorization"] = f"Bearer {api_key}"
    ...
    def _filter_popular_models(self, models: list) -> list:
        ...
        for model in models:
            ...
            if date_pattern.search(model_id):
                logger.info(f"⏭️ 跳过带日期的旧版本: {model_id}")
                continue
            ...
            if has_exclude_keyword:
                logger.info(f"⏭️ 跳过排除关键词: {model_id}")
                continue

            # 保留该模型
            logger.info(f"✅ 保留模型: {model_id}")
            filtered.append(model)
