...
class OperationLogService:
    ...
    async def create_log(
        ...
        try:
            db = get_mongo_db()
            if db is None:
                logger.debug(f"MongoDB unavailable, skipping operation log: {username} - {log_data.action}")
                return "no-db"
            ...
            logger.info(f"Operation log recorded: {username} - {log_data.action}")
    ...
    async def get_stats(self, days: int = 30) -> OperationLogStats:
        try:
            db = get_mongo_db()
            if db is None:
                return OperationLogStats(total_logs=0, unique_users=0, action_type_counts={}, daily_counts=[], top_actions=[])
    ...
    async def clear_logs(self, days: Optional[int] = None, action_type: Optional[str] = None) -> Dict[str, Any]:
        try:
            db = get_mongo_db()
            if db is None:
                return {"deleted_count": 0, "message": "MongoDB unavailable"}
