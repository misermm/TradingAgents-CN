...
# 硬编码的热门A股列表（沪深300获取失败时的备用列表）
_FALLBACK_CN_HOT_STOCKS = [
    "600519", "000858", "601318", "600036", "000333",
    "600276", "601166", "000651", "600030", "601888",
    "002415", "600900", "601012", "000002", "600809",
    "601669", "002304", "600585", "000568", "601688",
    "600887", "000725", "601398", "600000", "601288",
    "600050", "601857", "600028", "601988", "600048",
    "000001", "600104", "601601", "600690", "002230",
    "600309", "000625", "601899", "600436", "002352",
    "601088", "600196", "000538", "600570", "603259",
    "601225", "600346", "000776", "601138", "600438",
]

# 美股热门列表：MAG7 + SPY/QQQ
_US_HOT_STOCKS = ["AAPL", "MSFT", "GOOGL", "AMZN", "NVDA", "META", "TSLA", "SPY", "QQQ"]

# 港股热门列表：恒生指数成分股前20只
_HK_HOT_STOCKS = [
    "00700", "09988", "00941", "00005", "01299",
    "00939", "03988", "02318", "00001", "02628",
    "00883", "00267", "01109", "02015", "00669",
    "01810", "02382", "00002", "00003", "00011",
]


def _is_cn_trading_hours() -> bool:
    """判断当前是否为A股交易时段（9:30-15:00 北京时间）"""
    now = datetime.now(UTC_8)
    # 周末不交易
    if now.weekday() >= 5:
        return False
    # 9:30-11:30, 13:00-15:00
    morning = now.hour == 9 and now.minute >= 30 or 10 <= now.hour <= 11
    afternoon = 13 <= now.hour <= 14 or (now.hour == 15 and now.minute == 0)
    return morning or afternoon


def _is_us_trading_hours() -> bool:
    """判断当前是否为美股交易时段（21:30-04:00 北京时间）"""
    now = datetime.now(UTC_8)
    hour = now.hour
    # 北京时间 21:30-23:59
    if hour == 21 and now.minute >= 30 or hour in (22, 23):
        return True
    # 北京时间 00:00-04:00
    if 0 <= hour <= 3 or (hour == 4 and now.minute == 0):
        return True
    return False


def _get_cn_hot_stocks() -> list:
    """
    获取沪深300成分股代码列表
    优先使用AKShare接口，失败时使用硬编码列表
    """
    try:
        import akshare as ak
        df = ak.stock_hs300_weight_csindex(symbol="000300")
        if df is not None and not df.empty:
            ...
            if codes:
                logger.info(f"📋 [预取] 从AKShare获取沪深300成分股: {len(codes)}只")
                return codes
    except Exception as e:
        logger.warning(f"⚠️ [预取] 从AKShare获取沪深300成分股失败: {e}，使用硬编码列表")

    return _FALLBACK_CN_HOT_STOCKS


def _prefetch_cn_quotes_sync():
    """同步执行：预取A股热门股票实时行情"""
    stocks = _get_cn_hot_stocks()
    success_count = 0
    fail_count = 0
    ...
    for symbol in stocks:
        try:
            ...
            from tradingagents.dataflows.cache.cache_config import generate_cache_key, detect_market, get_ttl_seconds
            ...
            if result and "❌" not in result:
                ...
                ttl = get_ttl_seconds(market, "stock_data")
                manager._save_cached_text(symbol, "stock_data", result, start_date, end_date)
                success_count += 1
            else:
                fail_count += 1
        except Exception as e:
            fail_count += 1
            logger.debug(f"⚠️ [预取-A股行情] {symbol} 失败: {e}")

    logger.info(f"📊 [预取-A股行情] 完成: 成功{success_count}只, 失败{fail_count}只, 共{len(stocks)}只")


def _prefetch_cn_fundamentals_sync():
    """同步执行：预取A股热门股票基本面数据"""
    stocks = _get_cn_hot_stocks()
    # 只预取前50只的基本面（避免API限流）
    stocks = stocks[:50]
    ...
    for symbol in stocks:
        try:
            ...
            manager = get_data_source_manager()
            result = manager.get_fundamentals_data(symbol)

            if result and "❌" not in result:
                market = detect_market(symbol)
                cache_key = generate_cache_key(
                    market=market, data_type="fundamentals", symbol=symbol,
                )
                ttl = get_ttl_seconds(market, "fundamentals")
                manager._save_cached_text(symbol, "fundamentals", result, data_source="prefetch")
                success_count += 1
            else:
                fail_count += 1
        except Exception as e:
            fail_count += 1
            logger.debug(f"⚠️ [预取-A股基本面] {symbol} 失败: {e}")

    logger.info(f"📊 [预取-A股基本面] 完成: 成功{success_count}只, 失败{fail_count}只, 共{len(stocks)}只")


def _prefetch_us_quotes_sync():
    """同步执行：预取美股热门股票行情"""
    stocks = _US_HOT_STOCKS
    success_count = 0
    fail_count = 0
    ...
    for symbol in stocks:
        try:
            ...
            fail_count += 1
            logger.debug(f"⚠️ [预取-美股行情] {symbol} 失败: {e}")

    logger.info(f"📊 [预取-美股行情] 完成: 成功{success_count}只, 失败{fail_count}只, 共{len(stocks)}只")


async def prefetch_cn_quotes():
    """定时任务：预取A股热门股票实时行情（交易时段每5分钟）"""
    if not _is_cn_trading_hours():
        logger.debug("⏭️ [预取-A股行情] 非A股交易时段，跳过")
        return
    try:
        logger.info("🚀 [预取-A股行情] 开始预取沪深300实时行情...")
        await asyncio.to_thread(_prefetch_cn_quotes_sync)
    except Exception as e:
        logger.warning(f"⚠️ [预取-A股行情] 预取任务异常（不影响系统运行）: {e}")


async def prefetch_cn_fundamentals():
    """定时任务：预取A股热门股票基本面数据（每日收盘后20:00）"""
    try:
        logger.info("🚀 [预取-A股基本面] 开始预取沪深300基本面数据...")
        await asyncio.to_thread(_prefetch_cn_fundamentals_sync)
    except Exception as e:
        logger.warning(f"⚠️ [预取-A股基本面] 预取任务异常（不影响系统运行）: {e}")


async def prefetch_us_quotes():
    """定时任务：预取美股热门股票行情（美股交易时段每5分钟）"""
    if not _is_us_trading_hours():
        logger.debug("⏭️ [预取-美股行情] 非美股交易时段，跳过")
        return
    try:
        logger.info("🚀 [预取-美股行情] 开始预取MAG7+ETF行情...")
        await asyncio.to_thread(_prefetch_us_quotes_sync)
    except Exception as e:
        logger.warning(f"⚠️ [预取-美股行情] 预取任务异常（不影响系统运行）: {e}")


def register_prefetch_jobs(scheduler: AsyncIOScheduler):
    """
    注册数据预取定时任务到调度器
    ...
    scheduler.add_job(
        prefetch_cn_quotes,
        IntervalTrigger(minutes=5, timezone=UTC_8),
        id="prefetch_cn_quotes",
        name="预取-A股热门行情",
        replace_existing=True,
        max_instances=1,  # 防止并发执行
    )
    logger.info("✅ [预取] A股热门行情预取任务已注册（每5分钟）")

    # A股基本面预取：每日收盘后20:00
    scheduler.add_job(
        prefetch_cn_fundamentals,
        CronTrigger(hour=20, minute=0, timezone=UTC_8),
        id="prefetch_cn_fundamentals",
        name="预取-A股基本面",
        replace_existing=True,
        max_instances=1,
    )
    logger.info("✅ [预取] A股基本面预取任务已注册（每日20:00）")

    # 美股行情预取：交易时段每5分钟
    scheduler.add_job(
        prefetch_us_quotes,
        IntervalTrigger(minutes=5, timezone=UTC_8),
        id="prefetch_us_quotes",
        name="预取-美股热门行情",
        replace_existing=True,
        max_instances=1,
    )
    logger.info("✅ [预取] 美股热门行情预取任务已注册（每5分钟）")
