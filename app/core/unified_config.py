...
class UnifiedConfigManager:
    ...
    def _get_model_from_db(self, key: str, fallback: str) -> str:
        try:
            from app.core.database import get_mongo_db_sync
            db = get_mongo_db_sync()
            doc = db.system_configs.find_one({"is_active": True}, sort=[("version", -1)])
            if doc and doc.get("system_settings"):
    ...
    def get_default_model(self) -> str:
        """获取默认模型（向后兼容）"""
        db_val = self._get_model_from_db("quick_analysis_model", "")
        if db_val:
            return db_val
    ...
    def get_deep_analysis_model(self) -> str:
        ...
        db_val = self._get_model_from_db("deep_analysis_model", "")
        if db_val:
            return db_val
        settings = self.get_system_settings()
