...
class Settings(BaseSettings):
    ...
    # ==================== LLM API密钥配置 ====================
    DASHSCOPE_API_KEY: str = Field(default="", description="阿里云灵积API Key")
    DEEPSEEK_API_KEY: str = Field(default="", description="DeepSeek API Key")
    DEEPSEEK_BASE_URL: str = Field(default="", description="DeepSeek自定义Base URL")
    ...
    AI302_API_KEY: str = Field(default="", description="AI302 API Key")
    SILICONFLOW_API_KEY: str = Field(default="", description="SiliconFlow API Key")
    QIANFAN_API_KEY: str = Field(default="", description="百度千帆 API Key")
    ONEAPI_API_KEY: str = Field(default="", description="OneAPI API Key")
    ONEAPI_BASE_URL: str = Field(default="", description="OneAPI Base URL")
    CUSTOM_OPENAI_API_KEY: str = Field(default="", description="自定义OpenAI兼容API Key")

    # ==================== LLM供应商启用开关 ====================
    DASHSCOPE_ENABLED: bool = Field(default=True, description="启用阿里云灵积")
    DEEPSEEK_ENABLED: bool = Field(default=True, description="启用DeepSeek")
    OPENAI_ENABLED: bool = Field(default=True, description="启用OpenAI")
    QIANFAN_ENABLED: bool = Field(default=False, description="启用百度千帆")
    GOOGLE_ENABLED: bool = Field(default=False, description="启用Google Gemini")
    ANTHROPIC_ENABLED: bool = Field(default=False, description="启用Anthropic Claude")
    OPENROUTER_ENABLED: bool = Field(default=False, description="启用OpenRouter")
    AIHUBMIX_ENABLED: bool = Field(default=False, description="启用AiHubMix")
    AI302_ENABLED: bool = Field(default=False, description="启用AI302")

    # ==================== 功能开关 ====================
    MONGODB_ENABLED: bool = Field(default=True, description="启用MongoDB存储")
    REDIS_ENABLED: bool = Field(default=True, description="启用Redis缓存")
    MEMORY_ENABLED: bool = Field(default=True, description="启用记忆系统")
    ONLINE_TOOLS_ENABLED: bool = Field(default=True, description="启用在线工具")
    ONLINE_NEWS_ENABLED: bool = Field(default=True, description="启用在线新闻")
    REALTIME_DATA_ENABLED: bool = Field(default=True, description="启用实时数据")

    # ==================== 数据源配置 ====================
    DEFAULT_CHINA_DATA_SOURCE: str = Field(default="akshare", description="中国A股默认数据源(akshare/tushare)")

    # ==================== 成本统计配置 ====================
    ENABLE_COST_TRACKING: bool = Field(default=True, description="启用成本追踪")
    COST_ALERT_THRESHOLD: float = Field(default=100.0, description="成本告警阈值(USD)")
    MAX_USAGE_RECORDS: int = Field(default=10000, description="最大使用记录数")
    USE_MONGODB_STORAGE: bool = Field(default=True, description="使用MongoDB存储统计数据")

    # ==================== Docker标识 ====================
    DOCKER_CONTAINER: bool = Field(default=False, description="是否在Docker容器中运行")
    RUNNING_IN_DOCKER: bool = Field(default=False, description="是否在Docker容器中运行(别名)")
