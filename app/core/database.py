...
_sync_mongo_retry_after: float = 0.0
...
def get_mongo_db() -> AsyncIOMotorDatabase:
    """获取MongoDB数据库实例"""
    if mongo_db is None:
        logger.warning("⚠️ MongoDB数据库未初始化，返回None")
        return None
    return mongo_db
...
def get_mongo_db_sync_if_available(cooldown_seconds: float = 60.0) -> Optional[Database]:
    """Return the shared sync MongoDB database only when it is reachable.

    When MongoDB is unavailable, cache that failure for a short cooldown so
    repeated fallback checks do not pay the full server-selection timeout cost.
    """
    global _sync_mongo_retry_after

    if not getattr(settings, "MONGODB_ENABLED", True):
        return None

    now = time.monotonic()
    if now < _sync_mongo_retry_after:
        return None

    try:
        db = get_mongo_db_sync()
        db.client.admin.command("ping")
        _sync_mongo_retry_after = 0.0
        return db
    except Exception as exc:
        _sync_mongo_retry_after = now + cooldown_seconds
        close_mongo_db_sync()
        logger.warning(
            "MongoDB sync access unavailable; skipping sync DB fallback for %.0fs: %s",
            cooldown_seconds,
            exc,
        )
        return None
...
def close_mongo_db_sync() -> None:
    """Close the shared sync MongoDB client used by non-async call sites."""
    global _sync_mongo_client, _sync_mongo_db
...
def get_redis_client() -> Redis:
    ...
    if redis_client is None:
        logger.warning("⚠️ Redis客户端未初始化，返回None")
        return None
    return redis_client
