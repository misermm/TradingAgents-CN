...
async def get_task_result(
    ...
    try:
        ...
        if not result_data:
            ...
            if mongo_result:
                ...
                result_data = {
                    ...
                    "report_audit": mongo_result.get("report_audit", {}),
                    "weighted_decision": mongo_result.get("weighted_decision", {}),
                    "cn_fact_snapshot": mongo_result.get("cn_fact_snapshot", {}),
        ...
        final_result_data = {
            ...
            "report_audit": safe_dict(result_data.get("report_audit")),
            "weighted_decision": safe_dict(result_data.get("weighted_decision")),
            "cn_fact_snapshot": safe_dict(result_data.get("cn_fact_snapshot")),
...
async def search_stocks(
    ...
    try:
        ...
        if market:
            market_map = {"cn": "A股", "us": "美股", "hk": "港股"}
            search_query["market"] = market_map.get(market, market)

        cursor = db.stock_basic_info.find(search_query).limit(20)
        results = []
...
async def get_popular_stocks(
    ...
    try:
        ...
        if market:
            market_map = {"cn": "A股", "us": "美股", "hk": "港股"}
            pipeline.insert(0, {"$match": {"market_type": market_map.get(market, market)}})

        popular = []
        async for doc in db.analysis_reports.aggregate(pipeline):
            symbol = doc["_id"]
            stock_name_doc = await db.stock_basic_info.find_one(
                {"$or": [{"symbol": symbol}, {"code": symbol}]}
            )
            popular.append({
                "symbol": symbol,
                "name": stock_name_doc.get("name", symbol) if stock_name_doc else symbol,
                "market": market or "cn",
                "current_price": 0,
                "change_percent": 0,
                "volume": 0,
...
async def share_analysis(
    ...
    return {
        "success": True,
        "data": {
            "share_url": f"/analysis/shared/{analysis_id}",
            "share_code": analysis_id[:8]
        },
        "message": "分享功能开发中"
    }
