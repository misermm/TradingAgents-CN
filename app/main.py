...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if user_service._db_available:
            await user_service.create_admin_user()
            logger.info("Admin user ensured")
        else:
            logger.info("MongoDB not available, using fallback admin account (admin/admin123)")
    except Exception as e:
    ...
    try:
        ...
        # 注册数据预取定时任务（Phase 2.1）
        try:
            from app.services.scheduler_service import register_prefetch_jobs
            register_prefetch_jobs(scheduler)
            logger.info("✅ 数据预取定时任务已注册")
        except Exception as e:
            logger.warning(f"⚠️ 数据预取定时任务注册失败（不影响系统运行）: {e}")
        ...
        logger.warning("⚠️ 调度器启动失败，应用将继续运行但定时任务不可用")
...
app.include_router(local_models_router.router, prefix="/api", tags=["local-models"])
