...
// 分析结果
export interface MasterDataQualityField {
  name: string
  status: 'present' | 'missing' | 'estimated'
  aliases?: string[]
  source?: string
...
export interface MasterDataQuality {
  ...
  estimated_required?: string[]
  field_quality: MasterDataQualityField[]
}

export interface MasterEvidenceRow {
  area: string
  rule: string
  field: string
  ...
  score: number
  max_score: number
  verdict: 'pass' | 'partial' | 'fail' | 'info'
  reason: string
}

export interface MasterQuantitativeResult {
  signal?: string
  score?: number
  max_score?: number
  details?: string
  evidence_table?: MasterEvidenceRow[]
}
