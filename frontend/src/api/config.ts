...
export interface LLMProvider {
  ...
  extra_config?: {
    has_api_key?: boolean
    source?: 'environment' | 'database' | 'local' | 'manual'
    [key: string]: any
  }
  // 🆕 聚合渠道支持
...
export interface LocalServiceScanResult {
  available: boolean
  base_url: string
...
export interface LocalModelScanResult {
  docker_environment: boolean
  ollama: LocalServiceScanResult
  lmstudio: LocalServiceScanResult
}
...
export const configApi = {
  ...
  // 扫描本地模型（Ollama / LM Studio）
  scanLocalModels(): Promise<LocalModelScanResult> {
    return unwrapResponse(ApiClient.get<LocalModelScanResult>('/api/local-models/scan'))
  }
}
