  ...
  <div class="config-management">
    ...
    <el-row :gutter="24">
      ...
      <el-col :span="4">
        <el-card class="config-menu" shadow="never">
          ...
          >
            ...
            <el-menu-item index="local-models">
      ...
      <el-col :span="20">
        ...
        <!-- 检测本地模型 -->
        <el-card v-show="activeTab === 'local-models'" class="config-content" shadow="never">
          <template #header>
            <div class="card-header">
              <h3>检测本地模型</h3>
              <el-button
                type="primary"
                @click="handleScanLocalModels"
                :loading="localModelsScanning"
              >
          ...
          <div class="local-models-content">
            <el-alert
              title="本地模型检测"
              type="info"
              description="扫描本机运行的 Ollama 和 LM Studio 服务，发现可用模型后可一键添加到大模型配置中。"
              :closable="false"
              style="margin-bottom: 20px;"
            />

            <!-- 扫描结果 -->
            <div v-if="localModelsResult" class="scan-results">
              <!-- Docker 环境提示 -->
              <el-tag
                v-if="localModelsResult.docker_environment"
                type="warning"
                style="margin-bottom: 16px;"
              >
                检测到 Docker 环境，已自动使用 host.docker.internal 连接宿主机服务
              </el-tag>

              <el-row :gutter="20">
                <!-- Ollama 结果 -->
                <el-col :span="12">
                  <div class="service-card" :class="{ 'service-available': localModelsResult.ollama.available, 'service-unavailable': !localModelsResult.ollama.available }">
                    <div class="service-header">
                      <div class="service-title">
                        <span class="service-icon">🦙</span>
                        <span class="service-name">Ollama</span>
                        <el-tag
                          :type="localModelsResult.ollama.available ? 'success' : 'danger'"
                          size="small"
                        >
                          {{ localModelsResult.ollama.available ? '在线' : '离线' }}
                        </el-tag>
                      </div>
                      <div class="service-url">{{ localModelsResult.ollama.base_url }}</div>
                    </div>

                    <div v-if="localModelsResult.ollama.available" class="service-models">
                      <div v-if="localModelsResult.ollama.models.length === 0" class="no-models">
                        未发现已加载的模型
                      </div>
                      <div
                        v-for="model in localModelsResult.ollama.models"
                        :key="model.id"
                        class="model-item"
                      >
                        <div class="model-info">
                          <span class="model-id">{{ model.id }}</span>
                          <el-tag size="small" type="info">{{ model.object }}</el-tag>
                        </div>
                        <el-button
                          size="small"
                          type="primary"
                          @click="addLocalModelToConfig('ollama', model.id, localModelsResult.ollama.base_url)"
                        >
                          一键添加
                        </el-button>
                      </div>
                    ...
                    <div v-else class="service-error">
                      ...
                      <span>{{ localModelsResult.ollama.error || '服务未启动或不可达' }}</span>
                    </div>
                  </div>
                </el-col>
                ...
                <!-- LM Studio 结果 -->
                <el-col :span="12">
                  <div class="service-card" :class="{ 'service-available': localModelsResult.lmstudio.available, 'service-unavailable': !localModelsResult.lmstudio.available }">
                    <div class="service-header">
                      <div class="service-title">
                        <span class="service-icon">🔬</span>
                        <span class="service-name">LM Studio</span>
                        <el-tag
                          :type="localModelsResult.lmstudio.available ? 'success' : 'danger'"
                          size="small"
                        >
                          {{ localModelsResult.lmstudio.available ? '在线' : '离线' }}
                        </el-tag>
                      </div>
                      <div class="service-url">{{ localModelsResult.lmstudio.base_url }}</div>
                    </div>

                    <div v-if="localModelsResult.lmstudio.available" class="service-models">
                      <div v-if="localModelsResult.lmstudio.models.length === 0" class="no-models">
                        未发现已加载的模型
                      </div>
                      <div
                        v-for="model in localModelsResult.lmstudio.models"
                        :key="model.id"
                        class="model-item"
                      >
                        <div class="model-info">
                        ...
                        <el-button
                          size="small"
                          type="primary"
                          @click="addLocalModelToConfig('lmstudio', model.id, localModelsResult.lmstudio.base_url)"
                        >
                          一键添加
                        </el-button>
                      </div>
                    ...
                    <div v-else class="service-error">
                      <el-icon><WarningFilled /></el-icon>
                      <span>{{ localModelsResult.lmstudio.error || '服务未启动或不可达' }}</span>
                    </div>
                  </div>
                </el-col>
              </el-row>
              ...
              <!-- 批量添加 -->
              <div v-if="allDiscoveredModels.length > 0" class="batch-actions">
                <el-divider content-position="left">批量操作</el-divider>
                <div class="batch-buttons">
                  <el-button type="primary" @click="addAllLocalModels">
                    <el-icon><Plus /></el-icon>
                    一键添加全部模型 ({{ allDiscoveredModels.length }})
                  </el-button>
                  <el-button @click="addOllamaModels" :disabled="ollamaModelCount === 0">
                    仅添加 Ollama ({{ ollamaModelCount }})
                  </el-button>
                  <el-button @click="addLmStudioModels" :disabled="lmStudioModelCount === 0">
                    仅添加 LM Studio ({{ lmStudioModelCount }})
                  </el-button>
                </div>
              </div>
            </div>
            ...
            <div v-else class="scan-placeholder">
              <el-empty description="点击开始扫描检测本机运行的本地模型服务">
                <el-button type="primary" @click="handleScanLocalModels" :loading="localModelsScanning">
                  <el-icon><Search /></el-icon>
                  开始扫描
                </el-button>
              </el-empty>
...
// 本地模型扫描相关
const localModelsScanning = ref(false)
const localModelsResult = ref<LocalModelScanResult | null>(null)

// 所有发现的模型（计算属性）
const allDiscoveredModels = computed(() => {
  if (!localModelsResult.value) return []
  const models: Array<{ provider: string; id: string; baseUrl: string }> = []
  if (localModelsResult.value.ollama.available) {
    localModelsResult.value.ollama.models.forEach(m => {
      models.push({ provider: 'ollama', id: m.id, baseUrl: localModelsResult.value!.ollama.base_url })
    })
  }
  if (localModelsResult.value.lmstudio.available) {
    localModelsResult.value.lmstudio.models.forEach(m => {
      models.push({ provider: 'lmstudio', id: m.id, baseUrl: localModelsResult.value!.lmstudio.base_url })
    })
  }
  return models
})
...
const ollamaModelCount = computed(() => {
  if (!localModelsResult.value?.ollama.available) return 0
  return localModelsResult.value.ollama.models.length
})

const lmStudioModelCount = computed(() => {
  if (!localModelsResult.value?.lmstudio.available) return 0
  return localModelsResult.value.lmstudio.models.length
})
...
const handleScanLocalModels = async () => {
  localModelsScanning.value = true
  try {
    const result = await configApi.scanLocalModels()
    localModelsResult.value = result

    const ollamaCount = result.ollama.available ? result.ollama.models.length : 0
    const lmStudioCount = result.lmstudio.available ? result.lmstudio.models.length : 0
    const totalCount = ollamaCount + lmStudioCount

    if (totalCount > 0) {
      ElMessage.success(`扫描完成，发现 ${totalCount} 个本地模型 (Ollama: ${ollamaCount}, LM Studio: ${lmStudioCount})`)
    } else {
      const availableServices = []
      if (result.ollama.available) availableServices.push('Ollama')
      if (result.lmstudio.available) availableServices.push('LM Studio')

      if (availableServices.length > 0) {
        ElMessage.info(`${availableServices.join('、')} 服务在线，但未发现已加载的模型`)
      } else {
        ElMessage.warning('未检测到本地模型服务，请确认 Ollama 或 LM Studio 已启动')
      }
    }
  } catch (error: any) {
    console.error('扫描本地模型失败:', error)
    ElMessage.error('扫描本地模型失败，请检查后端服务是否正常运行')
  } finally {
    localModelsScanning.value = false
  }
}

// 将本地模型添加到大模型配置
const addLocalModelToConfig = async (provider: string, modelId: string, baseUrl: string) => {
  try {
    // 确保对应的厂家存在
    await ensureLocalProviderExists(provider)

    // 构建 API base URL（Ollama 需要 /v1 后缀以兼容 OpenAI 格式）
    let apiBase = baseUrl
    ...
    const config: Partial<LLMConfig> = {
      provider: provider,
      model_name: modelId,
      model_display_name: `${provider === 'ollama' ? 'Ollama' : 'LM Studio'} - ${modelId}`,
      api_base: apiBase,
      max_tokens: 4096,
      temperature: 0.7,
      timeout: 120,
      retry_times: 3,
      enabled: true,
      description: `通过"检测本地模型"自动添加的 ${provider === 'ollama' ? 'Ollama' : 'LM Studio'} 模型`,
      capability_level: 3,
      suitable_roles: ['both'],
    ...
    ElMessage.success(`模型 ${modelId} 已添加到大模型配置`)

    // 刷新大模型配置列表
    await loadLLMConfigs()
  } catch (error: any) {
    console.error('添加本地模型失败:', error)
    ElMessage.error(error.message || '添加本地模型失败')
  }
}

// 确保本地模型厂家存在
const ensureLocalProviderExists = async (provider: string) => {
  const existing = providers.value.find(p => p.name === provider)
  if (existing) return

  const providerInfo: Record<string, Partial<LLMProvider>> = {
    ollama: {
      name: 'ollama',
      display_name: 'Ollama',
      description: '本地大模型运行框架，支持多种开源模型，无需 API Key',
      is_active: true,
    ...
    lmstudio: {
      ...
      description: '本地大模型运行工具，兼容 OpenAI API 格式，无需 API Key',
      is_active: true,
      supported_features: ['chat', 'completion'],
  ...
  if (info) {
    await configApi.addLLMProvider(info)
    ElMessage.success(`已自动创建 ${info.display_name} 厂家配置`)
    await loadProviders()
...
// 批量添加所有本地模型
const addAllLocalModels = async () => {
  const models = allDiscoveredModels.value
  if (models.length === 0) return

  try {
    await ElMessageBox.confirm(
      `确定要添加全部 ${models.length} 个本地模型到大模型配置吗？`,
      '批量添加确认',
      { type: 'info' }
    )
    ...
    for (const model of models) {
      try {
        await addLocalModelToConfigSilent(model.provider, model.id, model.baseUrl)
        successCount++
      } catch {
        failCount++
      }
    ...
    if (successCount > 0) {
      ElMessage.success(`批量添加完成：成功 ${successCount} 个${failCount > 0 ? `，失败 ${failCount} 个` : ''}`)
      await loadLLMConfigs()
    }
  } catch {
    // 用户取消
...
// 仅添加 Ollama 模型
const addOllamaModels = async () => {
  if (!localModelsResult.value?.ollama.available) return
  const models = localModelsResult.value.ollama.models
  if (models.length === 0) return

  try {
    await ElMessageBox.confirm(
      `确定要添加 ${models.length} 个 Ollama 模型吗？`,
      '批量添加确认',
      { type: 'info' }
    )
    ...
    for (const model of models) {
      try {
        await addLocalModelToConfigSilent('ollama', model.id, localModelsResult.value!.ollama.base_url)
        successCount++
      } catch {
        // 忽略单个失败
      }
    ...
    if (successCount > 0) {
      ElMessage.success(`已添加 ${successCount} 个 Ollama 模型`)
      await loadLLMConfigs()
    }
  } catch {
    // 用户取消
...
// 仅添加 LM Studio 模型
const addLmStudioModels = async () => {
  if (!localModelsResult.value?.lmstudio.available) return
  const models = localModelsResult.value.lmstudio.models
  if (models.length === 0) return

  try {
    await ElMessageBox.confirm(
      `确定要添加 ${models.length} 个 LM Studio 模型吗？`,
      '批量添加确认',
      { type: 'info' }
    )
    ...
    for (const model of models) {
      try {
        await addLocalModelToConfigSilent('lmstudio', model.id, localModelsResult.value!.lmstudio.base_url)
        successCount++
      } catch {
        // 忽略单个失败
      }
    ...
    if (successCount > 0) {
      ElMessage.success(`已添加 ${successCount} 个 LM Studio 模型`)
      await loadLLMConfigs()
    }
  } catch {
    // 用户取消
...
// 静默添加本地模型（不弹消息，用于批量操作）
const addLocalModelToConfigSilent = async (provider: string, modelId: string, baseUrl: string) => {
  await ensureLocalProviderExists(provider)
