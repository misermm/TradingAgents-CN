        ...
        <el-col :span="18">
          <el-card class="main-form-card" shadow="hover">
            ...
            <el-form :model="analysisForm" label-width="100px" class="analysis-form">
              ...
              <!-- 分析师团队 -->
              <div class="form-section">
                <h4 class="section-title">👥 专业分析师</h4>
              ...
              <!-- 投资大师 -->
              <div class="form-section">
                <h4 class="section-title">🎩 投资大师</h4>
                <div class="analysts-grid master-grid">
                  <div
                    v-for="master in MASTER_ANALYSTS"
                    :key="master.id"
                    class="analyst-card master-card"
                    :class="{ active: analysisForm.selectedAnalysts.includes(master.name) }"
                    @click="toggleAnalyst(master.name)"
                  >
                    <div class="analyst-avatar master-avatar">
                      <el-icon><User /></el-icon>
                    </div>
                    <div class="analyst-content">
                      <div class="analyst-name">{{ master.name }}</div>
                      <div class="analyst-desc">{{ master.description }}</div>
                    </div>
                    <div class="analyst-check">
                      <el-icon v-if="analysisForm.selectedAnalysts.includes(master.name)" class="check-icon">
                        <Check />
                      </el-icon>
                    </div>
                  </div>
          ...
          <el-col :span="24">
            <el-card class="results-card" shadow="hover">
              ...
              <div class="results-content">
                ...
                <!-- 最终决策 -->
                <div v-if="masterDataQualityEntries.length" class="master-quality-section">
                  <div class="master-quality-header">
                    <h4>大师分析数据完整度</h4>
                    <el-tag type="info" size="small">字段级校验</el-tag>
                  </div>
                  <div class="master-quality-grid">
                    <div
                      v-for="item in masterDataQualityEntries"
                      :key="item.id"
                      class="master-quality-card"
                    >
                      <div class="quality-card-head">
                        <span class="quality-master-name">{{ item.name }}</span>
                        <el-tag
                          :type="item.quality.is_sufficient ? 'success' : 'warning'"
                          size="small"
                        >
                          {{ item.quality.is_sufficient ? '可分析' : '数据不足' }}
                        </el-tag>
                      </div>
                      <el-progress
                        :percentage="item.percentage"
                        :status="item.quality.is_sufficient ? 'success' : 'warning'"
                        :stroke-width="8"
                      />
                      <div class="quality-meta">
                        <span>{{ item.quality.present_count }}/{{ item.quality.required_count }} 项真实可用</span>
                        <span v-if="item.quality.estimated_required?.length">估算 {{ item.quality.estimated_required.length }} 项</span>
                        <span v-if="item.quality.missing_required.length">缺失 {{ item.quality.missing_required.length }} 项</span>
                      </div>
                      <div v-if="item.sources.length" class="quality-sources">
                        <span class="source-label">免费源</span>
                        <el-tag
                          v-for="source in item.sources"
                          :key="`${item.id}-source-${source}`"
                          size="small"
                          effect="plain"
                        >
                          {{ source }}
                      ...
                      <div
                        v-if="item.quality.missing_required.length || item.quality.estimated_required?.length"
                        class="quality-fields"
                      >
                        <el-tag
                          v-for="field in item.quality.missing_required"
                        ...
                        <el-tag
                          v-for="field in item.quality.estimated_required || []"
                          :key="`${item.id}-estimated-${field}`"
                          type="warning"
                          size="small"
                          effect="plain"
                        >
                ...
                <div v-if="masterEvidenceEntries.length" class="master-evidence-section">
                  <div class="master-quality-header">
                    <h4>大师规则证据</h4>
                    <el-tag type="success" size="small">量化规则</el-tag>
                  </div>
                  <div class="master-evidence-grid">
                    <div
                      v-for="item in masterEvidenceEntries"
                      :key="item.id"
                      class="master-evidence-card"
                    >
                      <div class="quality-card-head">
                        <span class="quality-master-name">{{ item.name }}</span>
                        <div class="evidence-head-meta">
                          <el-tag :type="getSignalTagType(item.signal)" size="small">
                            {{ getSignalLabel(item.signal) }}
                          </el-tag>
                          <span class="evidence-score">{{ item.scoreText }}</span>
                        </div>
                      </div>
                      ...
                      <div class="evidence-list">
                        <div
                          ...
                          :key="`${item.id}-${row.field}-${row.rule}`"
                          class="evidence-row"
                        >
                          <div class="evidence-row-head">
                            <div class="evidence-rule-block">
                              <span class="evidence-area">{{ row.area }}</span>
                              <span class="evidence-rule">{{ row.rule }}</span>
                            </div>
                            <el-tag :type="getEvidenceVerdictTagType(row.verdict)" size="small">
                              {{ getEvidenceVerdictLabel(row.verdict) }}
                            </el-tag>
                          </div>

                          <div class="evidence-row-body">
                            <span class="evidence-field">{{ row.field }}</span>
                            <span class="evidence-observed">{{ row.observed }}</span>
                            <span class="evidence-points">{{ row.score }}/{{ row.max_score }}</span>
                          </div>

                          <div class="evidence-row-meta">
                            <span v-if="row.source" class="evidence-source">来源 {{ row.source }}</span>
                            <span v-if="row.report_period" class="evidence-source">报告期 {{ row.report_period }}</span>
                            <span v-if="row.updated_at" class="evidence-source">更新 {{ row.updated_at }}</span>
                          </div>

                          <p v-if="row.reason" class="evidence-reason">{{ row.reason }}</p>
                        </div>
                      </div>
                    </div>
                  </div>
...
import type { MasterDataQuality, MasterDataQualityField, MasterEvidenceRow, MasterQuantitativeResult } from '@/types/analysis'
...
import { ANALYSTS, MASTER_ANALYSTS, convertAnalystNamesToIds } from '@/constants/analysts'
import { REPORT_MAPPINGS } from '@/constants/reports'
...
const masterDataQualityEntries = computed(() => {
  const result = analysisResults.value || {}
  const qualityMap = (result.master_data_quality || result.state?.master_data_quality || {}) as Record<string, MasterDataQuality>

  return Object.entries(qualityMap)
    .filter(([, quality]) => quality && quality.is_applicable !== false && quality.required_count > 0)
    .map(([id, quality]) => {
      const master = MASTER_ANALYSTS.find(item => item.id === id)
      const sources = Array.from(new Set(
        (quality.field_quality || [])
          .map(field => field.source)
          .filter((source): source is string => Boolean(source && source !== 'none'))
      ))
      return {
        id,
        name: master?.name || id,
        quality,
        sources,
        percentage: Math.max(0, Math.min(100, Math.round((quality.completeness || 0) * 100)))
      }
    })
})

const normalizeEvidenceField = (value?: string) => (value || '').trim().toLowerCase()

const buildQualityFieldIndex = (fields: MasterDataQualityField[] = []) => {
  const index = new Map<string, MasterDataQualityField>()
...
const masterEvidenceEntries = computed(() => {
  const result = analysisResults.value || {}
  const quantitativeMap = (result.master_quantitative_results || result.state?.master_quantitative_results || {}) as Record<string, MasterQuantitativeResult>
  const qualityMap = (result.master_data_quality || result.state?.master_data_quality || {}) as Record<string, MasterDataQuality>

  return Object.entries(quantitativeMap)
    .map(([id, quantitative]) => {
      const rows = Array.isArray(quantitative?.evidence_table) ? quantitative.evidence_table : []
      if (!rows.length) {
        return null
      }
      ...
      const fieldIndex = buildQualityFieldIndex(quality?.field_quality || [])
      const enrichedRows = rows.map((row: MasterEvidenceRow) => {
        const metadata = fieldIndex.get(normalizeEvidenceField(row.field))
        return {
          ...row,
      ...
      return {
        id,
        name: master?.name || id,
        signal: quantitative?.signal || 'neutral',
        scoreText: `${quantitative?.score ?? 0}/${quantitative?.max_score ?? 0}`,
        rows: enrichedRows
      }
    })
...
const getEvidenceVerdictTagType = (verdict?: string) => {
...
const getEvidenceVerdictLabel = (verdict?: string) => {
...
const getAnalysisReports = (data: any) => {
  ...
  const reportMappings = REPORT_MAPPINGS
...
const getReportDescription = (title: string) => {
  const descMap: Record<string, string> = {
    ...
    '📈 A股市场分析': 'A股市场技术指标、行业板块、资金流向分析',
    '💰 基本面分析': '财务数据、估值水平、盈利能力分析',
    '📰 新闻事件分析': '相关新闻事件、市场动态影响分析',
    '💭 市场情绪分析': '投资者情绪、社交媒体情绪指标',
    '🎩 巴菲特': '价值投资：护城河+安全边际+能力圈分析',
    '🎩 彼得·林奇': 'GARP策略：增长+估值+基本面综合分析',
    '🎩 格雷厄姆': '深度价值：安全边际+财务实力+盈利稳定性',
    '🎩 芒格': '质量优先：护城河+管理质量+可预测性',
    '🎩 凯瑟琳·伍德': '颠覆性创新：技术潜力+创新增长+高增长估值',
    '🎩 阿克曼': '激进投资：业务质量+财务纪律+催化剂',
    '🎩 费舍尔': '成长投资：增长质量+利润率+管理效率',
    '🎩 德鲁肯米勒': '宏观趋势：增长动量+风险回报+估值',
    '🎩 达莫达兰': '估值专家：生命周期+DCF估值+情景分析',
    '🎩 布瑞': '逆向投资：深度价值+安全边际+风险信号',
    '🎩 帕伯莱': '低风险套利：不对称机会+巴芒过滤器',
    '🎩 塔勒布': '黑天鹅防御：脆弱性+反脆弱+杠铃策略',
    '🎩 朱朱瓦拉': '新兴市场：宏观趋势+成长性+估值时机',
    '📊 大师共识': '多位投资大师的综合共识分析',
    '🐂 多头研究员': '看多视角的论证分析',
    '🐻 空头研究员': '看空视角的风险提示',
    '🔬 研究经理决策': '综合多空观点的研究决策',
...
// 获取深度描述
const getDepthDescription = (depth: number): '快速' | '基础' | '标准' | '深度' | '全面' => {
  const descriptions = ['快速', '基础', '标准', '深度', '全面']
  return (descriptions[depth - 1] || '标准') as '快速' | '基础' | '标准' | '深度' | '全面'
}
...
.master-evidence-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(320px, 1fr));
  gap: 14px;
}

.master-evidence-card {
  border: 1px solid #e5e7eb;
  border-radius: 8px;
  padding: 14px;
  background: #f9fafb;
