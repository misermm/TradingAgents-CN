          ...
          <el-card class="config-card" shadow="hover">
            ...
            <el-form :model="batchForm" label-width="100px" class="batch-form">
              ...
              <div class="form-section">
                ...
                <div class="analysts-selection">
                  <el-checkbox-group v-model="batchForm.analysts" class="analysts-group master-group">
                    <div
                      v-for="master in MASTER_ANALYSTS"
                      :key="master.id"
                      class="analyst-option master-option"
                    >
                      <el-checkbox :label="master.name" class="analyst-checkbox master-checkbox">
                        <div class="analyst-info">
                          <span class="analyst-name">{{ master.name }}</span>
                          <span class="analyst-desc">{{ master.description }}</span>
                        </div>
                      </el-checkbox>
                    </div>
                  </el-checkbox-group>
...
import { ANALYSTS, MASTER_ANALYSTS, DEFAULT_ANALYSTS, convertAnalystNamesToIds } from '@/constants/analysts'
...
const submitBatchAnalysis = async () => {
  ...
  try {
    ...
    // 准备批量分析请求参数（真实API调用）
    const detectedMarkets = new Set(symbols.value.map(s => getMarketByStockCode(s)))
    const marketType = detectedMarkets.size === 1 ? Array.from(detectedMarkets)[0] as 'A股' | '美股' | '港股' : 'A股'

    const researchDepth = batchForm.depth as '快速' | '基础' | '标准' | '深度' | '全面'
    const language = batchForm.language as 'zh-CN' | 'en-US'
      ...
      .analysts-selection {
        .analysts-group {
          ...
          .analyst-option {
            ...
            &.master-option .analyst-checkbox {
              :deep(.el-checkbox__input.is-checked .el-checkbox__inner) {
