...
def create_llm_by_provider(provider: str, model: str, backend_url: str, temperature: float, max_tokens: int, timeout: int, api_key: str = None, **extra_kwargs):
    ...
    if normalized_provider in {"openai", "siliconflow", "openrouter", "aihubmix", "ollama", "lmstudio", "deepseek", "qwen", "glm", "custom_openai", "qianfan"}:
        if not api_key:
...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        Args:
            selected_analysts: List of analyst types to include. Options are:
                Regular analysts: "market", "social", "news", "fundamentals"
                Master analysts: "warren_buffett", "peter_lynch", "ben_graham",
                    "charlie_munger", "cathie_wood", "bill_ackman",
        ...
        elif normalized_provider in {"openai", "siliconflow", "openrouter", "aihubmix", "ollama", "lmstudio"}:
    ...
    def propagate(self, company_name, trade_date, progress_callback=None, task_id=None):
            ...
            for chunk in self._safe_stream(init_agent_state, args):
                ...
                if progress_callback and args.get("stream_mode") == "updates":
                    ...
                    for node_name, node_update in chunk.items():
                        if not node_name.startswith('__'):
                            if not isinstance(node_update, dict):
                                logger.warning(f"⚠️ 节点 {node_name} 返回非字典类型: {type(node_update)}, 跳过状态更新")
                                continue
                            node_update = _merge_master_state(final_state, node_update)
        ...
        # 从 checkpointer 获取真实的最终状态
        # 之前没有使用 checkpointer，graph.get_state() 无法可靠返回状态
        # 现在使用 MemorySaver + thread_id，可以获取完整的最终状态
        try:
            graph_state = self.graph.get_state(args.get("config", {}))
            if graph_state and graph_state.values:
                real_final_state = dict(graph_state.values)
                real_master_reports = real_final_state.get("master_reports", {})
                acc_master_reports = final_state.get("master_reports", {})
                real_count = sum(1 for v in real_master_reports.values() if isinstance(v, str) and len(v.strip()) > 10)
                acc_count = sum(1 for v in acc_master_reports.values() if isinstance(v, str) and len(v.strip()) > 10)

                logger.info(f"📊 [STATE] checkpointer真实状态: {real_count}条大师报告")
                logger.info(f"📊 [STATE] 手动累积状态: {acc_count}条大师报告")
                logger.info(f"📊 [STATE] checkpointer中master_reports keys: {list(real_master_reports.keys())}")
                logger.info(f"📊 [STATE] 累积状态中master_reports keys: {list(acc_master_reports.keys())}")

                # 详细对比每个大师的报告
                for master_id in real_master_reports:
                    real_val = real_master_reports.get(master_id, "")
                    acc_val = acc_master_reports.get(master_id, "")
                    real_len = len(real_val.strip()) if isinstance(real_val, str) else 0
                    acc_len = len(acc_val.strip()) if isinstance(acc_val, str) else 0
                    if real_len != acc_len:
                        logger.warning(f"⚠️ [STATE] {master_id}: checkpointer长度={real_len}, 累积长度={acc_len}")

                if real_count > acc_count:
                    logger.warning(f"⚠️ [STATE FIX] 手动累积state丢失了大师报告，从checkpointer真实state恢复!")
                    # 使用 checkpointer 的真实状态覆盖手动累积的状态
                    # 优先使用非空值
                    merged = dict(acc_master_reports)
                    for k, v in real_master_reports.items():
                        # 如果累积状态中为空但checkpointer中有值，使用checkpointer的值
                        if k in merged and merged[k] and not v:
                            continue
                        # 如果checkpointer的值比累积状态的值更长，使用checkpointer的值
                        if isinstance(v, str) and isinstance(merged.get(k, ""), str):
                            if len(v.strip()) > len(merged.get(k, "").strip()):
                                merged[k] = v
                        else:
                            merged[k] = v
                    final_state["master_reports"] = merged
                    logger.info(f"🎯 [STATE FIX] 恢复大师报告: {real_count}条")

                    # 同时恢复 master_tool_call_counts
                    real_tool_counts = real_final_state.get("master_tool_call_counts", {})
                    acc_tool_counts = final_state.get("master_tool_call_counts", {})
                    merged_counts = dict(acc_tool_counts)
                    for k, v in real_tool_counts.items():
                        if k in merged_counts and merged_counts.get(k, 0) > 0 and not v:
                            continue
                        if isinstance(v, int) and v > merged_counts.get(k, 0):
                            merged_counts[k] = v
                    final_state["master_tool_call_counts"] = merged_counts

                elif acc_count > 0:
                    logger.info(f"✅ [STATE FIX] 大师报告在累积state中正常: {acc_count}条")
                else:
                    logger.warning(f"⚠️ [STATE FIX] checkpointer真实state中也没有大师报告!")
            else:
                logger.warning(f"⚠️ [STATE FIX] 无法获取checkpointer真实state")
        except Exception as e:
            logger.warning(f"⚠️ [STATE FIX] 获取graph真实state失败: {e}")
        ...
        master_reports = final_state.get("master_reports", {})
        master_report_count = sum(1 for v in master_reports.values() if isinstance(v, str) and len(v.strip()) > 10)
        if master_report_count > 0:
            logger.info(f"🎯 [MASTER REPORT] final_state 中有 {master_report_count} 条大师报告")
        else:
            logger.warning(f"⚠️ [MASTER REPORT] final_state 中没有大师报告!")
    ...
    def _send_progress_update(self, chunk, progress_callback):
        ...
        try:
            ...
            node_mapping = {
                ...
                # 大师分析师节点
                'Warren Buffett Analyst': "🎩 巴菲特大师分析",
                'Peter Lynch Analyst': "🎩 彼得·林奇大师分析",
                'Ben Graham Analyst': "🎩 格雷厄姆大师分析",
                'Charlie Munger Analyst': "🎩 芒格大师分析",
                'Cathie Wood Analyst': "🎩 凯瑟琳·伍德大师分析",
                'Bill Ackman Analyst': "🎩 阿克曼大师分析",
                'Phil Fisher Analyst': "🎩 费舍尔大师分析",
                'Druckenmiller Analyst': "🎩 德鲁肯米勒大师分析",
                'Aswath Damodaran Analyst': "🎩 达莫达兰大师分析",
                'Michael Burry Analyst': "🎩 布瑞大师分析",
                'Mohnish Pabrai Analyst': "🎩 帕伯莱大师分析",
                'Nassim Taleb Analyst': "🎩 塔勒布大师分析",
                'Rakesh Jhunjhunwala Analyst': "🎩 朱朱瓦拉大师分析",
                # 大师分析师工具节点
                'tools_warren_buffett': None,
                'tools_peter_lynch': None,
                'tools_ben_graham': None,
                'tools_charlie_munger': None,
                'tools_cathie_wood': None,
                'tools_bill_ackman': None,
                'tools_phil_fisher': None,
                'tools_stanley_druckenmiller': None,
                'tools_aswath_damodaran': None,
                'tools_michael_burry': None,
                'tools_mohnish_pabrai': None,
                'tools_nassim_taleb': None,
                'tools_rakesh_jhunjhunwala': None,
                # 大师分析师消息清理节点
                'Msg Clear Warren Buffett': None,
                'Msg Clear Peter Lynch': None,
                'Msg Clear Ben Graham': None,
                'Msg Clear Charlie Munger': None,
                'Msg Clear Cathie Wood': None,
                'Msg Clear Bill Ackman': None,
                'Msg Clear Phil Fisher': None,
                'Msg Clear Druckenmiller': None,
                'Msg Clear Aswath Damodaran': None,
                'Msg Clear Michael Burry': None,
                'Msg Clear Mohnish Pabrai': None,
                'Msg Clear Nassim Taleb': None,
                'Msg Clear Rakesh Jhunjhunwala': None,
                # 大师共识节点
                'Data PreFetch': "📡 预获取数据",
                'Master Consensus': "📊 大师共识汇总",
                # 研究员节点
    ...
    def _log_state(self, trade_date, final_state):
        ...
        for master_id in MASTER_ANALYST_IDS:
            ...
            if report:
                log_data[f"{master_id}_report"] = report
        ...
        self.log_states_dict[str(trade_date)] = log_data
