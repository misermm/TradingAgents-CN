...
def _running_in_real_docker() -> bool:
    return Path("/.dockerenv").exists()


def _normalize_log_dir(path_value: str) -> str:
    raw = (path_value or "./logs").strip()
    candidate = Path(raw)
    if raw.startswith("/app/") and not _running_in_real_docker():
        return str((_PROJECT_ROOT / candidate.name).resolve())
    if candidate.is_absolute():
        return str(candidate)
    return str((_PROJECT_ROOT / candidate).resolve())


def _normalize_log_file(path_value: str) -> str:
    raw = (path_value or "").strip()
    if not raw:
        return raw
    candidate = Path(raw)
    if raw.startswith("/app/") and not _running_in_real_docker():
        return str((_PROJECT_ROOT / "logs" / candidate.name).resolve())
    if candidate.is_absolute():
        return str(candidate)
    return str((_PROJECT_ROOT / candidate).resolve())
...
class TradingAgentsLogger:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        log_dir = _normalize_log_dir(os.getenv('TRADINGAGENTS_LOG_DIR', './logs'))
        ...
        for config_path in config_paths:
            if config_path and Path(config_path).exists():
                try:
                    ...
                    # 转换配置格式
                    converted = self._convert_toml_config(config_data)
                    for handler_name in ("file", "error", "structured"):
                        handler_cfg = converted.get("handlers", {}).get(handler_name)
                        if handler_cfg and handler_cfg.get("directory"):
                            handler_cfg["directory"] = _normalize_log_dir(handler_cfg["directory"])
                        if handler_cfg and handler_cfg.get("filename"):
                            handler_cfg["filename"] = _normalize_log_file(handler_cfg["filename"])
                    return converted
