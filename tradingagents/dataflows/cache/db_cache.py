...
class DatabaseCacheManager:
    ...
    def __init__(self,
                 ...
                 mongodb_db: str = "tradingagentscn",
                 redis_db: int = 0):
        """
        初始化数据库缓存管理器
        ...
        if mongodb_url:
            ...
            self.mongodb_url = f"mongodb://{mongodb_username}:{mongodb_password}@localhost:{mongodb_port}/?authSource={mongodb_auth_source}"
        else:
            self.mongodb_url = os.getenv("MONGODB_URL", f"mongodb://localhost:{mongodb_port}")
        ...
        if redis_url:
            ...
            self.redis_url = os.getenv("REDIS_URL", f"redis://localhost:{redis_port}")
