...
def calculate_realtime_pe_pb(
    ...
    try:
        ...
        if 'AsyncIOMotorClient' in client_type or 'Motor' in client_type:
            ...
            logger.debug(f"检测到异步客户端 {client_type}，转换为同步连接池")
        ...
        market_quotes = _get_collection(db, "market_quotes")
        stock_basic_info = _get_collection(db, "stock_basic_info")

        quote = market_quotes.find_one({"code": code6})
        if not quote:
        ...
        basic_info = stock_basic_info.find_one({"code": code6, "source": "tushare"})

        if not basic_info:
            # 🔥 诊断：查看 MongoDB 中有哪些数据源
            all_sources = list(stock_basic_info.find({"code": code6}, {"source": 1, "_id": 0}))
            ...
            basic_info = stock_basic_info.find_one({"code": code6})
            if not basic_info:
        ...
        financial_data = stock_financial_data.find_one({"code": code6}, sort=[("report_period", -1)])
        pb = None
...
def get_pe_pb_with_fallback(
    ...
    try:
        ...
        if 'AsyncIOMotorClient' in client_type or 'Motor' in client_type:
            from app.core.database import get_mongo_db_sync
            logger.debug(f"检测到异步客户端 {client_type}，转换为同步连接池")
            db_client = get_mongo_db_sync().client
