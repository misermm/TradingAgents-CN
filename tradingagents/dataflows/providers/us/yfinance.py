...
_yfinance_client = ResilientHttpClient(
    max_retries=3,
...
class YFinanceUtils:
    ...
    def get_stock_data(
        ...
        result = _yfinance_client.call_sync_with_timeout(
            ticker.history, start=start_date, end=end_date, timeout=45.0
        )

        if not result.success:
            logger.error(f"❌ yfinance获取数据失败: {result.error}")
            return DataFrame()
