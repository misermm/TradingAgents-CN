...
class TushareProvider(BaseStockDataProvider):
    ...
    def _get_token_from_database(self) -> Optional[str]:
        ...
        try:
            ...
            from app.core.database import get_mongo_db_sync_if_available
            db = get_mongo_db_sync_if_available()
            if db is None:
                self.logger.info("MongoDB unavailable; skipping Tushare token lookup")
                return None
    ...
    async def get_stock_quotes(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            realtime_api = getattr(self.api, "realtime_quote", None)
            if callable(realtime_api):
                try:
                    realtime_df = await asyncio.to_thread(realtime_api, ts_code=ts_code)
                    if realtime_df is not None and not realtime_df.empty:
                        realtime_row = realtime_df.iloc[0].to_dict()
                        realtime_row.setdefault("ts_code", ts_code)
                        realtime_row.setdefault("symbol", symbol)
                        return self.standardize_quotes(realtime_row)
                except Exception as realtime_error:
                    self.logger.warning(f"实时行情接口失败，回退到 daily/daily_basic: {realtime_error}")
            ...
            daily_basic_df = None
            daily_basic_api = getattr(self.api, "daily_basic", None)
            if callable(daily_basic_api):
                try:
                ...
                except Exception as daily_basic_error:
                    self.logger.warning(f"daily_basic 获取失败: {daily_basic_error}")
            ...
            if df is not None and not df.empty:
                ...
                return self.standardize_quotes(row)
