...
class AdaptiveRateLimiter:
    ...
    功能：
    - 监控最近10次请求的平均响应时间
    - 响应时间>2s时自动将请求间隔从0.5s增加到1s/2s
    - 响应时间恢复正常后逐步降低间隔
    - 遇到429/403响应时立即进入冷却期（60s）
    """
    
    # 请求间隔档位（秒）
    INTERVAL_LEVELS = [0.5, 1.0, 2.0, 3.0]
    # 响应时间阈值（秒）
    SLOW_THRESHOLD = 2.0
    # 冷却期（秒）
    COOLDOWN_SECONDS = 60
    ...
    def __init__(self):
        """初始化自适应频率限制器"""
        self._response_times = deque(maxlen=self.WINDOW_SIZE)  # 最近10次响应时间
        self._current_interval = self.INTERVAL_LEVELS[0]  # 当前请求间隔
        self._interval_index = 0  # 当前间隔档位索引
        self._last_request_time = 0.0  # 上次请求时间
        self._cooldown_until = 0.0  # 冷却期结束时间
        self._lock = threading.Lock()  # 线程安全锁
        logger.info(f"🔧 自适应频率限制器初始化，默认间隔: {self._current_interval}s")
    ...
    def record_response(self, response_time: float, status_code: int = 200):
        ...
        with self._lock:
            ...
            if status_code in (429, 403):
                ...
                logger.warning(f"⚠️ [频率限制] 收到 {status_code} 响应，进入冷却期 {self.COOLDOWN_SECONDS}s")
                return
    ...
    def _adjust_interval(self):
        ...
        if avg_time > self.SLOW_THRESHOLD:
            # 响应时间过长，增加间隔
            if self._interval_index < len(self.INTERVAL_LEVELS) - 1:
                self._interval_index += 1
                self._current_interval = self.INTERVAL_LEVELS[self._interval_index]
                logger.info(f"🔧 [频率限制] 平均响应时间 {avg_time:.2f}s > {self.SLOW_THRESHOLD}s，增加间隔到 {self._current_interval}s")
        elif avg_time < self.SLOW_THRESHOLD * 0.5:
            # 响应时间恢复正常，逐步降低间隔
            if self._interval_index > 0:
                self._interval_index -= 1
                self._current_interval = self.INTERVAL_LEVELS[self._interval_index]
                logger.info(f"🔧 [频率限制] 平均响应时间 {avg_time:.2f}s 恢复正常，降低间隔到 {self._current_interval}s")
    ...
    def wait_if_needed(self):
        ...
        with self._lock:
            ...
            if now < self._cooldown_until:
                ...
                logger.info(f"🔒 [频率限制] 冷却期中，等待 {wait_time:.1f}s")
            ...
            if elapsed < self._current_interval:
                ...
                logger.debug(f"🔧 [频率限制] 等待 {wait_time:.2f}s（当前间隔: {self._current_interval}s）")
                time.sleep(wait_time)
    ...
    async def async_wait_if_needed(self):
        ...
        with self._lock:
            ...
            if now < self._cooldown_until:
                wait_time = self._cooldown_until - now
                logger.info(f"🔒 [频率限制] 冷却期中，异步等待 {wait_time:.1f}s")
            else:
                elapsed = now - self._last_request_time
                if elapsed < self._current_interval:
                    wait_time = self._current_interval - elapsed
...
class AKShareProvider(BaseStockDataProvider):
    ...
    # 关键数据类型的备选接口注册表
    # 格式: {数据类型: [(接口名称, 接口方法), ...]}
    API_REDUNDANCY_REGISTRY = {
        "realtime_quotes": [
            ("stock_bid_ask_em", "stock_bid_ask_em"),       # 主接口：单股实时盘口
            ("stock_zh_a_spot", "stock_zh_a_spot"),         # 备选1：新浪全市场快照
            ("stock_zh_a_spot_em", "stock_zh_a_spot_em"),   # 备选2：东方财富全市场快照
            ("stock_zh_a_hist", "stock_zh_a_hist"),         # 兜底：最新日线
        ],
        "historical_data": [
            ("stock_zh_a_hist", "stock_zh_a_hist"),             # 主接口：东方财富历史行情
            ("stock_zh_a_hist_min_em", "stock_zh_a_hist_min_em"), # 备选：分钟级行情（可聚合）
        ],
        "daily_basic": [
            ("stock_zh_a_spot_em", "stock_zh_a_spot_em"),               # 主接口：东方财富实时行情（含PE/PB等）
            ("stock_financial_analysis_indicator", "stock_financial_analysis_indicator"), # 备选：财务分析指标
            ("stock_a_indicator_lg", "stock_a_indicator_lg"),           # 备选2：乐咕乐股指标（PE/PB/股息率）
        ],
        "stock_info": [
            ("stock_individual_info_em", "stock_individual_info_em"),   # 主接口：个股信息
            ("stock_info_a_code_name", "stock_info_a_code_name"),       # 备选：股票列表
        ],
    }
    ...
    def __init__(self):
        ...
        self._rate_limiter = AdaptiveRateLimiter()  # 自适应频率限制器
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    self._rate_limiter.record_response(response_time)
    ...
    async def get_historical_data(
        ...
        try:
            ...
            api_methods = [
                ("stock_zh_a_hist", lambda: self.ak.stock_zh_a_hist(
                    symbol=code,
                    period=ak_period,
                ...
                ("stock_zh_a_daily", lambda: self.ak.stock_zh_a_daily(
                    symbol=code,
                    start_date=start_date_formatted,
                    end_date=end_date_formatted,
                    adjust="qfq"
            ...
            def fetch_historical_data():
                return self._try_multiple_apis(api_methods)

            hist_df = await asyncio.to_thread(fetch_historical_data)

            if hist_df is None or hist_df.empty:
                logger.warning(f"⚠️ {code}历史数据为空（所有接口均失败）")
                return None
