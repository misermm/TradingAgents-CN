...
def get_realtime_stock_news(ticker: str, curr_date: str, hours_back: int = 6) -> str:
    ...
    if is_china_stock:
        logger.info(f"[新闻分析] ========== 步骤2: 实时新闻聚合器 ==========")
        ...
        try:
            ...
            if news_items and len(news_items) > 0:
                return aggregator.format_news_report(news_items, ticker)
        except Exception as e:
            logger.error(f"[新闻分析] 实时新闻聚合器获取失败: {e}，继续尝试其他新闻源")
    ...
    if is_china_stock:
        ...
        try:
            logger.info(f"[新闻分析] 尝试通过兼容层获取东方财富新闻")
            ...
            if not news_df.empty:
                ...
                # 记录一些新闻标题示例
                sample_titles = [
                    row.get('新闻标题') or row.get('标题') or '无标题'
                ...
                for idx, (_, row) in enumerate(news_df.iterrows()):
                    if idx < 3:  # 只记录前3条的详细信息
                        logger.info(
                            f"[新闻分析] 第{idx+1}条新闻: "
                            f"标题={row.get('新闻标题') or row.get('标题') or '无标题'}, "
                            f"时间={row.get('发布时间') or row.get('时间') or '无时间'}"
                        )
                    report += f"### {row.get('新闻标题') or row.get('标题') or ''}\n"
                    report += f"📅 {row.get('发布时间') or row.get('时间') or ''}\n"
                    report += f"🔗 {row.get('新闻链接') or row.get('链接') or ''}\n\n"
                    report += f"{row.get('新闻内容') or row.get('内容') or '无内容'}\n\n"
