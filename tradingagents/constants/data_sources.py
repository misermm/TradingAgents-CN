...
class DataSourceCode(str, Enum):
    ...
    SINA = "sina"            # 新浪财经 - 免费A股/港股实时行情
    EASTMONEY = "eastmoney"  # 东方财富直接API - 免费A股行情/财务/行业数据
...
DATA_SOURCE_REGISTRY: Dict[str, DataSourceInfo] = {
    ...
    # 新浪财经
    DataSourceCode.SINA: DataSourceInfo(
        code=DataSourceCode.SINA,
        name="SinaFinance",
        display_name="新浪财经",
        provider="新浪财经",
        description="新浪财经免费API，提供A股和港股实时行情，稳定运行10+年",
        supported_markets=["a_shares", "hk_stocks"],
        requires_api_key=False,
        is_free=True,
        official_website="http://finance.sina.com.cn",
        features=["实时行情", "历史K线", "港股行情", "完全免费", "低延迟"],
    ),
    
    # 东方财富直接API
    DataSourceCode.EASTMONEY: DataSourceInfo(
        code=DataSourceCode.EASTMONEY,
        name="EastMoneyDirect",
        display_name="东方财富(直连)",
        provider="东方财富",
        description="直接调用东方财富公开API，绕过AKShare中间层，支持批量行情和财务数据",
        supported_markets=["a_shares"],
        requires_api_key=False,
        is_free=True,
        official_website="http://quote.eastmoney.com",
        features=["实时行情", "批量行情", "财务数据", "行业数据", "完全免费"],
    ),
