...
def create_msg_delete():
    def delete_messages(state):
        """Add placeholder message for next node compatibility.
        
        In LangGraph parallel branches, multiple Msg Clear nodes may try to
        delete the same shared messages, causing "Attempting to delete a message
        with an ID that doesn't exist" errors. To avoid this, we simply add
        a placeholder message instead of deleting. The report content is already
        saved in the master_reports dict, so
        message cleanup is not critical for correctness.
        """
...
"""
    ...
    def get_stock_fundamentals_unified(
        ...
        if research_depth == "快速":
            data_depth = "standard"
            logger.info(f"🔧 [分析级别] 快速分析模式：获取标准数据（含护城河指标）")
        ...
        try:
            ...
            if data_depth == "basic":
                ...
                logger.info(f"📊 [基本面策略] 快速分析模式：获取标准财务指标（含ROE/毛利率等护城河指标）")
            ...
            if is_china:
                ...
                try:
                    ...
                    try:
                        ...
                        source_payloads = collect_china_free_source_payloads(str(ticker))
                        source_payloads.extend([
                            {"source": "text", "data": current_price_data, "updated_at": curr_date},
                            {"source": "text", "data": fundamentals_data, "updated_at": curr_date},
                        ])
                        snapshot = build_china_fundamental_snapshot(
                            str(ticker),
                            source_payloads,
                        )
                        result_data.append(format_china_fundamental_snapshot_report(snapshot))
                        logger.info(
                            f"✅ [A股免费数据融合] quality={snapshot['quality']['present_count']}/{snapshot['quality']['total_count']}"
                        )
                    except Exception as snapshot_error:
                        logger.warning(f"⚠️ [A股免费数据融合] 生成质量报告失败: {snapshot_error}")
                except Exception as e:
