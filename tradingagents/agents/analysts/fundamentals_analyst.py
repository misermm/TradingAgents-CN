...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            "- 结合市场特点进行分析"
            "🔍 数据一致性要求（重要）："
            "- 如果EPS为负，PE必须标注为N/A或负值，绝不能报告正PE"
            "- 如果PB异常高(>100)，必须标注并说明原因（如净资产接近零）"
            "- 如果ROE与净利率符号不一致，必须在报告中明确标注矛盾"
            "- 如果现金流与净利润符号不一致，必须标注盈利质量存疑"
            "- 所有数据必须标注来源，对矛盾数据必须标注⚠️并解释"
            "- 禁止使用与快照数据矛盾的数据，如快照显示EPS为负则不得声称PE为正"
            "🌍 语言和货币要求："
            "- 所有分析内容必须使用中文"
            "- 投资建议必须使用中文：买入、持有、卖出"
        ...
        try:
            ...
            logger.error(f"❌ [基本面分析师] LLM调用失败(已重试): {err_type}: {str(llm_err)[:200]}")
