...
确保在报告末尾附上Markdown表格总结关键发现和投资建议。"""
        ...
        if GoogleToolCallHandler.is_google_model(llm):
            ...
            if len(tool_calls) == 0:
                ...
                logger.info(f"📈 [中国市场分析师] 检测到工具调用，执行工具并生成报告")
                try:
                    ...
                    logger.info(f"📈 [中国市场分析师] 工具调用后生成报告，长度: {len(report)}")
                except Exception as e:
                    logger.error(f"📈 [中国市场分析师] 工具调用处理失败: {e}")
                    report = result.content if result.content else "中国市场分析完成，但报告生成失败。"
